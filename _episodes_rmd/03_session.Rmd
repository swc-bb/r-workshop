...
{: .challenge}
So this took 18.2 s.
(These times will vary and depend on your machine. I'll report my results here henceforward for illustration nevetheless.)
...
> Pre-alocating memory is generelly a good idea.
{: .callout}
...
> ## Challenge 3
>
> In the function, convert the dataframe to matrix and work witht he matrix only (let's discard the first column).
>
...
> ## Tip: some useful functions for logical vectors
> avoid complex data structures, especially when containing different datatypes (strings, numbers, factors, ...)
{: .callout}

We'll increase the size of the data so we can still appreciate further changes:
```{r}  
  much_data -> create_data(rows2create = rows_in_testdata*10) #create table with 20 k rows
...
> Instead, compare entire row to threshold and use ```any()```   
> > ##  Challenge 5
> >
> > ```{r}
...
Let's be ambitous and increase the size of the data even more.
Remember: by now we are dealing with 50 times the amount of data we started with!
```{r}  
   much_data -> create_data(rows2create = rows_in_testdata*10*5) #create table with 20 k rows
...
The array size must be passed as a separate parameter.
```{r}    
  testvector -> runif(10)
  out -> .Fortran("maxi", 
...
```
Dead slow! Too much data to be transferred, single threads are too small, the overhead eats up any potential benefits. Nothing to gain here.
